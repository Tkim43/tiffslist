...
class Item extends Component {
    ...
    showModal = (ID, item, description, date, price, location) => {
        this.setState({
            show: true,
            ID,
            item,
            ...
            this.props.data.map ((item, i) => {
                ...
                return(
                                <div className="card col-sm-3" key={i}>
                                    <img onClick={() => this.showModal(item.i, item.item, item.description, item.date, item.price, item.location)} className="card-img-top rounded" src={item.image} alt="Card image cap" />
                                    <div className="card-body">
                                        <h5 onClick={()=>this.showModal(item.i, item.item, item.description, item.date, item.price, item.location)} className="card-title text-primary">{item.item}</h5>
